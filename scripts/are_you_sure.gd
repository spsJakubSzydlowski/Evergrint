...
var options = load("res://scenes/UI/menu/options_sect.tscn")
...
func _ready() -> void:
	...
	if visible and Enums.MenuActions.RESET_BINDS:
		...
		are_you_sure_label.text = "Delete World?"
...
func _on_yes_button_pressed() -> void:
	...
	if Enums.MenuActions.RESET_BINDS:
		...
		if Global.world_name: 
			var opt = options.instantiate()
			get_parent().add_child(opt)
			queue_free()
		else: Signals.switch_to_section.emit("options")
	else:
...
func _on_visibility_changed() -> void:
	if not are_you_sure_label: return
	
	if visible and Enums.current_menu_action == Enums.MenuActions.RESET_BINDS:
		are_you_sure_label.text = "Reset to Default?"
	else:
		are_you_sure_label.text = "Delete World?"
